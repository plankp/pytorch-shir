...
with open(f"./metrics/lenet_4/{PROFILE}_{PROBLEM_SIZE_N}.log", "w") as f:
  for i in range(PROBLEM_TRIPS):
    rows = time_inference(test_dataloader, model)
    # print(i, " / ", PROBLEM_TRIPS)
    for w in rows:
      print(w, file=f)
    # f.flush()
