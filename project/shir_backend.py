...
class SHIRGraphModule(torch.nn.Module):
  ...
  def _emit(self, f, clname):
    ...
    print("        algo.Map({", file=f)
    print("          val _1 = core.ParamDef(algo.TupleType(seq, algo.SignedIntType(32)))", file=f)
    print("          algo.AlgoLambda(Seq(_1),", file=f)
    print("            // trunc(u32 + at least u32) -> u32, sext(u32) -> s32", file=f)
    print("            algo.Sub(algo.Tuple(algo.TruncInteger(", file=f)
    print("              algo.Add2(algo.TruncInteger(algo.Select(core.ParamUse(_1), 1), 32),", file=f)
    print("                // u32 + u32 + ... -> at least u32", file=f)
    print("                algo.Fold(Add2.asFunction(), algo.Map({", file=f)
    ...
    print("                      // s8 * s8 -> s16; sext(16) -> s32; trunc(s32) -> u32", file=f)
    print("                      algo.Mul(core.ParamUse(_0)), algo.ConstantInteger(0, Some(algo.SignedIntType(32)))), 32))", file=f)
    print("                }, algo.Zip(algo.Tuple(core.ParamUse(_0), algo.Select(core.ParamUse(_1), 0)))))),", file=f)
    print("              32), algo.ConstantInteger(0))))", file=f)
    print("        }, algo.Zip(algo.Tuple(rhs, acc))))", file=f)
    print("    }, lhs)", file=f)
    print("  }", file=f)
    print("}", file=f)
