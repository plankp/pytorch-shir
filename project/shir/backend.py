...
# some debug flags
_FLAG_EMIT_SHIR_CODE = True
_FLAG_EMIT_DATA_FILES = True
...
class SHIRGraphModule(torch.nn.Module):
  ...
  def __call__(self, *args):
    ...
    if _FLAG_EMIT_DATA_FILES:
      data_dir = f"data_{self._call_id}"
      if not os.path.exists(data_dir):
        os.mkdir(data_dir)

      for i, arg in enumerate(args):
        with open(f"{data_dir}/arg{i}.csv", "w", encoding="utf-8") as argf:
          _tensor_to_matrix_csv(arg, argf)

      with open(f"{data_dir}/result.csv", "w", encoding="utf-8") as resf:
        _tensor_to_matrix_csv(result, resf)

    return result

  def compile(self):
    if not self._compiled and _FLAG_EMIT_SHIR_CODE:
      self._force_compile()
      self._compiled = True
  ...
  def _force_compile(self):
    ...
    with open(f"{clname}.scala", "w", encoding="utf-8") as f:
      ...
      print("    HDLProject(\"boom\", _synth_ir())", file=f)
      print("      .simulateWithHostRam(_read_inputs(args(0)))", file=f)
