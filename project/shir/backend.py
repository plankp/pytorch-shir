...
class SHIRGraphModule(torch.nn.Module):
  ...
  def _emit_source(self):
    ...
    with open(os.path.join(
      ...
      print("import backend.hdl.sim.ModelSimExec", file=f)
      ...
      print("import java.io.File", file=f)
      print(file=f)

      print("object", clname, "{", file=f)
      print(file=f)
      print("  def main(args: Array[String]): Unit = {", file=f)
      print("    val project = HDLProject(\"", clname, "\", _synth_ir())", sep="", file=f)
      print("    project.writeAllFiles(_read_inputs(args(0)))", file=f)
      print("    ModelSimExec.run(project.PROJECT_FOLDER, false)", file=f)
      print("      .assertCorrect", file=f)
      print("      .print()", file=f)
      print("  }", file=f)
      print(file=f)
      ...
      print("    val f = Source.fromFile(fname, \"UTF-8\")", file=f)
